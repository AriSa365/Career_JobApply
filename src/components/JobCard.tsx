import { Bookmark, BookmarkCheck, Building2, CalendarDays, ExternalLink, MapPin } from 'lucide-react'
  ...
  return (
    <article className="job-card">
      ...
      <div className="job-footer">
        <div className="source-copy">Found via {job.via || 'Google Jobs'} · strict 30-day filter passed</div>
        <div className="job-actions">
          <button className="icon-btn" onClick={onToggleSave} title={saved ? 'Remove saved job' : 'Save job'}>
            {saved ? <BookmarkCheck size={18} /> : <Bookmark size={18} />}
          </button>
          <a className="apply-btn" href={job.applyUrl} target="_blank" rel="noreferrer">Open application <ExternalLink size={15} /></a>
        </div>
      </div>
    </article>
  )
